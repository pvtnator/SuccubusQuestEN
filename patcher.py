...
if __name__ == "__main__":
    ...
    source = sys.argv[2] if len(sys.argv)>2 else "SQ_Patch.rb"
    quickpatch = ""
    translated_dir = current_dir.parent / Path().resolve().name.replace("patch","translated")
    source_dir = current_dir.parent / Path().resolve().name.replace("_patch","")
    ...
    if mode == "extract":
        extract_strings(script_source, translations_file, translations, conv)
        print("Extraction completed.")
    elif mode == "apply":       
        apply_translations(script_dest, script_dest, regexes, translations, quickpatch)
